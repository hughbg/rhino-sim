...
class sparse_beam(UVBeam):
    ...
    def __init__(self, filename, nmax, mmodes, za_range=(0, 90), save_fn='', 
                 ...
                 alpha=np.sqrt(1 - np.cos(46 * np.pi / 90)), Nfeeds=None, **kwargs):
        """
        ...
        #print("Init sparse beam", filename, nmax, mmodes)
    ...
    def interp(self, sparse_fit=False, fit_coeffs=None, az_array=None, 
               za_array=None, freq_array=None, **kwargs):
        """
        ...
        """
        #print("sparse interp", None if az_array is None else az_array.size, None if za_array is None else za_array.size, freq_array)
        ...
        if sparse_fit:
            ...
            if freq_array is None:
                ...
                # Find nearest neighbour index for start/finish
                idx_start = (np.abs(self.freq_array[0]-freq_array[0])).argmin()
                idx_finish = (np.abs(self.freq_array[0]-freq_array[-1])).argmin()

                #print(idx_start, idx_finish, self.freq_array, self.freq_array[0, idx_start:idx_finish+1])
